...
import { Shield, Eye, Flame, Search, VenetianMask, Heart, Skull, X } from 'lucide-react';
import './Edo.css';
...
export default function EdoGame({ gameStateData }: { gameStateData: any }) {
  ...
  const renderReveal = (rData: any, isPrivate: boolean) => (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
      <div className="bg-[#111] border border-gray-800 max-w-md w-full p-8 text-center shadow-2xl animate-in zoom-in-95 duration-500">
        ...
        {isPrivate ? (
            <div className="space-y-4">
                ...
                <div className="text-4xl font-bold text-white uppercase tracking-widest mt-2">{rData.alignment}</div>
                {rData.exactRole && <div className="text-red-500 mt-2">True Role: {rData.exactRole}</div>}
            </div>
        ) : (
            <div className="space-y-4">
      ...
      <div className="flex-1 flex flex-col md:flex-row relative z-10 overflow-hidden">
        ...
        <div className={`fixed md:relative bottom-0 right-0 w-full md:w-80 lg:w-96 h-[50vh] md:h-full transform ${isMobileScrollOpen ? 'translate-y-0' : 'translate-y-[calc(100%-3rem)]'} md:translate-y-0 transition-transform duration-500 z-40 flex flex-col bg-black/60 backdrop-blur-md md:border-l border-t md:border-t-0 border-gray-800 shadow-2xl`}>
            ...
            <div className="flex-1 w-full flex flex-col overflow-hidden relative">
                ...
                <div className={`p-4 border-t border-gray-800 bg-[#111] ${(gameState !== 'day_discussion' || activePlayer?.isDead) ? 'opacity-50' : ''}`}>
                    <form onSubmit={submitNote} className="flex gap-2">
                        <input 
                            ...
                            className="flex-1 bg-transparent border border-gray-700 px-3 py-2 outline-none text-white placeholder-gray-600 text-sm transition-colors focus:border-red-500" 
                        ...
                        />
                        <button type="submit" disabled={gameState !== 'day_discussion' || activePlayer?.isDead} className="px-3 bg-white/5 border border-gray-700 text-gray-300 hover:text-white hover:border-gray-500 transition-colors disabled:opacity-50 flex items-center justify-center">
